"""
Road flow choropleth + city overlay + filtered EV chargers — Spain (2022).
...
Layers (bottom → top):
  ...
  4. EV chargers that fall within a city boundary (cyan dots)
"""
...
CSV_CHARGERS   = os.path.join(HERE, "..", "data", "processed", "chargers_in_cities.csv")
OUT_PNG        = os.path.join(HERE, "spain_flow_cities_filtered_chargers_2022.png")
...
def main():
    ...
    print("Loading filtered chargers…")
    cx_ch, cy_ch = load_chargers(CSV_CHARGERS)
    print(f"  {len(cx_ch):,} chargers (within cities only)")
    ...
    ax.scatter([], [], s=10, color="#00e5ff", alpha=0.8, label=f"EV charger in city ({len(cx_ch):,})")
    ax.legend(loc="lower left", framealpha=0.3, facecolor="#0f1117",
              edgecolor="white", labelcolor="white", fontsize=9)

    ax.set_title(
        "Spain — AADT by Road Segment, Cities & EV Chargers within Cities  (2022)",
